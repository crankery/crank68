...
; equates 
	.equ stack, 0x017f
	.equ piaadr, 0xe208
	.equ pfilbg, 0x0102 ; start of loaded file
	.equ pfilen, 0x0104 ; end of loaded file
	.equ extern, 0x1f00 ; ptr to where the EXTERNAL command JSRs to
						; don't have anything for that yet so just reset
	.equ monitr, 0xe27b ; location to return to monitor (same as reset vector)
	.equ monpc, 0xe27b
	.equ stkbot, 0x0100
...
; the normal setup for this code is to put start right here at 0x100
; we'll just set it to the byte following the "lable table" so it doesn't
; blow away the start of whatever's at 0x100
...
outeee:	jmp 0xe175 ; mikbug outch?
inch:	jsr 0xe178 ; mikbug inch
break:	jmp fakebrk ; originally "intbrk", requires PIA.
memend:	.word 0x3fff
